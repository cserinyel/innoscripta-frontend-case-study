 ...
 */
const SOURCE_TO_CATEGORY_MAP: Record<string, string> = Object.entries(
  NEWSAPI_CATEGORY_BY_SOURCE_MAP,
).reduce<Record<string, string>>((acc, [category, sources]) => {
...
/**
 * Resolves a category from a NewsAPI source ID using the reverse lookup map.
 * Falls back to "general" when the source ID is unknown.
 */
export const getCategoryBySourceId = (
  sourceId: string | null,
): string => {
  if (!sourceId) return "general";
  return SOURCE_TO_CATEGORY_MAP[sourceId] ?? "general";
};
